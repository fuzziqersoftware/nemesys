...
#define INFIX_ERROR(e) {Variable(ValueType::Int, static_cast<int64_t>(e)), \
    shared_ptr<Variable>(new Variable(ValueType::Unicode, L ## #e))}

unordered_map<Variable, shared_ptr<Variable>> errno_names({
  // common errors
  INFIX_ERROR(E2BIG),
  INFIX_ERROR(EACCES),
...
static map<string, Variable> globals({
  ...
  {"errorcode",              Variable(ValueType::Dict, errno_names)},

#define INFIX_ERROR(e) {#e, Variable(ValueType::Int, static_cast<int64_t>(e))}
  // common errors
  INFIX_ERROR(E2BIG),
  INFIX_ERROR(EACCES),
  INFIX_ERROR(EADDRINUSE),
...
std::shared_ptr<ModuleAnalysis> errno_module(new ModuleAnalysis("posix", globals));
