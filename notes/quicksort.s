...
//  this follows the system v calling convention (sort of). call it with the
// array pointer in rdi and the number of elements in the array in rsi. it does
// not return anything; rax is meaningless after calling it
.globl quicksort
quicksort:
  // we use rdx as the array ptr everywhere (in recursive calls too). in the
  // rest of the implementation, rdi is the start index and rsi is the end index
  mov   rdx, rdi
...
1:
  ...
  // now we loop over the values and separate them into low and high buckets.
  // r8 will point to the next value to be bucketed, and r9 will point to the
  // index between the two buckets. start with r8 = r9 = start, and decrement r8
  // first so we can increment it at the beginning of the loop.
  lea   r8, [rdi - 1]
  mov   r9, rdi
2:
  ...
  // a[r9] into the low bucket, and moves the first element in the high bucket
  // to the end of the high bucket.
  mov   rcx, [rdx + r9 * 8]
  xchg  rcx, [rdx + r8 * 8]
  mov   [rdx + r9 * 8], rcx
  inc   r9
...
3:
  ...
  // args onto the stack since we can not recompute them after the first call
  push  rsi
  lea   rax, [r9 + 1]
  push  rax
