...
; return anything; don't assume the value of rax is useful after calling it.
.globl quicksort
quicksort:
  ; we'll use rdx as the array ptr everywhere (in recursive calls too). in the
  ; rest of this implementation, rdi is the start index and rsi is the end index
  mov rdx, rdi
...
1:
  ...
  ; move the pivot to the end, leaving pivot_value in rax (we'll need it later)
  mov rax, [rdx + rsi * 8]
  xchg rax, [rdx + rcx * 8]
  mov [rdx + rsi * 8], rax
...
3:
  ...
  ; we're done bucketing. swap a[end] and a[j] to put the pivot value between
  ; the buckets, so we can recur on both buckets. (rax is still pivot_value)
  xchg rax, [rdx + r9 * 8]
  mov [rdx + rsi * 8], rax
